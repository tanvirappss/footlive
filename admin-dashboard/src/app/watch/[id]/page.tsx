...
export default function UserWatchPage() {
  ...
  useEffect(() => {
    ...
    for (const event of commentaryData.keyEvents) {
      ...
      if (isNotifiable) {
        ...
        const homeN = match ? (match.home_team_id ? match.home_team?.name : match.home_team_custom_name) : '';
        const awayN = match ? (match.away_team_id ? match.away_team?.name : match.away_team_custom_name) : '';
        const matchTitle = homeN && awayN ? `${homeN} vs ${awayN}` : '';
  ...
  }, [commentaryData, isCommentaryInitialized, systemConfig, match]);
  ...
  return (
    <div className="min-h-screen bg-[#090c10] text-[#f0f3f8] flex flex-col">
      ...
      {/* Floating active toasts overlay - top-right (won't cover video player) */}
